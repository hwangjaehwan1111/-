    ...
    try:
        ...
        for _, s_row in df_results.iterrows():
            ...
            with PdfPages(pdf_buffer) as pdf:
                ...
                ax1 = fig.add_axes([0.11, 0.52, 0.32, 0.22], polar=True)
                ...
                path_effects=[path_effects.SimpleLineShadow(shadow_color='#888888', alpha=0.6, offset=(2, -2)), path_effects.Normal()])
                ...
                ax2 = fig.add_axes([0.56, 0.53, 0.35, 0.22])
                ...
                ax2.set_ylim(0, max_val * 1.15) # 숫자 라벨을 위한 위쪽 여백
                ...
                ax2.set_yticks([]) 
                fig.text(0.27, 0.78, "▶ 영역별 핵심 역량 지표 (%)", fontsize=14, fontweight='bold', color=COLOR_NAVY, ha='center')
                fig.text(0.735, 0.78, "▶ 단원별 성취도", fontsize=14, fontweight='bold', color=COLOR_NAVY, ha='center')
                ...
                fig.text(0.19, 0.44, f" {student_name} 학생 심층 분석", fontsize=15, fontweight='bold', color=COLOR_NAVY)
                ...
                score_gap = s_ordered.max() - s_ordered.min()
                if avg_val >= 95 or score_gap <= 5:
                    best_worst_text = f"영역별 분석 결과, 모든 영역에서 편차 없이 고르게 탁월한 학업 밸런스를 유지하고 있습니다. 지금의 훌륭한 학습 습관과 몰입도를 꾸준히 이어가면서, 더 넓고 깊은 심화 학습으로 지적 호기심을 확장해 나가길 적극 권장합니다."
                    worst_solution = "지금처럼 꾸준히 올바른 학습 태도를 유지한다면, 앞으로의 중등 과정에서도 최상위권의 자리를 굳건히 지킬 수 있을 것입니다."
                else:
                    best_worst_text = f"분석 결과, '{best_cat}' 영역에서 남다른 강점이 돋보이는 반면, 상대적으로 '{worst_cat}' 역량에서는 집중적인 보완이 이루어진다면 한 차원 더 높은 도약이 기대됩니다. 특히 이번 평가에서 '{worst_unit}' 단원의 오답률이 다소 눈에 띄므로, 해당 단원의 핵심 개념을 반드시 짚고 넘어가야 합니다."
                    worst_solution = solution_dict.get(worst_cat, "악어수학의 꼼꼼한 맞춤 클리닉을 통해 부족한 부분을 채워 나간다면 충분히 더 큰 성장을 이뤄낼 수 있습니다.")

                diag_content = (
                    f"1. 종합 진단: {student_name} 학생은 전체 평균({total_avg_val}%) 대비 성취도 {avg_val}%를 기록하며, 현재 [{eval_tier}]를 보여주고 있습니다.\n\n"
                    f"2. 강약점 분석: {best_worst_text}\n\n"
                    f"3. JEET 전문가 맞춤 솔루션: {worst_solution}"
                )
